...
trait ListOperations[F[_], Model] { this: PartialCrudService[F] =>
   ...
   */
  def stream(pairs: (String, String)*): Stream[F, Model] = stream(Query.fromPairs(pairs*))
   ...
   */
  def stream(query: Query, extraHeaders: Header.ToRaw*): Stream[F, Model] = stream[Model](pluralName, uri.copy(query = query), extraHeaders*)
   ...
   * @param pairs the pairs of Strings that will be used as the query params.
   */
  def list(pairs: (String, String)*): F[List[Model]] = list(Query.fromPairs(pairs*))
   ...
   * @param extraHeaders extra headers to pass when making the request. The `authToken` header is always added.
   */
  def list(query: Query, extraHeaders: Header.ToRaw*): F[List[Model]] = list[Model](pluralName, uri.copy(query = query), extraHeaders*)
}